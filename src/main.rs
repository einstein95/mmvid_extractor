...
fn main() {
    ...
    let filename = &args[1];
    let mut mmfile = fs::OpenOptions::new()
        .read(true)
        .write(true)
        .open(filename)
    ...
    mmfile
        .write_all(b"\x00\x00\x00\x01skip")
        .expect("Unable to write skip atom header!");
    mmfile
        .seek(SeekFrom::Start(0x10))
        .expect("Unable to seek past header!");
    ...
    println!(
        "Version: {}, Number of file offsets: {}",
        version, num_offsets
    );
    ...
    for _ in 0..num_offsets {
        let offset = mmfile
            .read_u32::<BigEndian>()
            .expect("Unable to read file offset!");
        offsets.push(offset);
    }
    let _filelength = mmfile
        .read_u32::<BigEndian>()
        .expect("Unable to read file length!");
    if filename.to_lowercase().ends_with(".mmq") {
        mmfile
            .seek(SeekFrom::Current(0x4))
            .expect("Unable to seek past junk offset!");
    }
...
fn convert(mmfile: &mut File, offset: u64, filename: &str, output_filename: &str) {
    ...
    mmfile
        ...
        .expect("Unable to seek to write new header!");
    mmfile
        .write_all(&length_to_header(offset))
        .expect("Unable to write size to new header!");
